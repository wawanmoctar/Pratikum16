...
from sim.mahasiswa.forms import mahasiswa_F, loginmahasiswa_F, pengaduan_F, editmahasiswa_F, edit_pengaduan_F
...
def data_m():
    ...
    if form.validate_on_submit():
        ...
        add_mahasiswa=Tmahasiswa(npm=form.npm.data, nama=form.nama.data, email=form.email.data, password=pass_hash, 
        kelas=form.kelas.data, alamat=form.alamat.data)
...
def login_mahasiswa():
    ...
    if form.validate_on_submit():
        ...
        if ceknpm and bcrypt.check_password_hash(ceknpm.password, form.password.data):
            ...
            flash('Login Gagal, Periksa NPM dan Password kembali!','primary')
...
def edit_mahasiswa():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Data Berhasil di Update','primary')
...
def pengaduan():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Selamat Data Berhasil Ditambahkan','warning')
...
@login_required
def edit_pengaduan(ed_id):
    form=edit_pengaduan_F()
...
@rmahasiswa.route("/hapus_pengaduan/<id>", methods=['GET','POST'])
def hapus_pengaduan(id):
        add_pengaduan=Tpengaduan.query.get(id)
        db.session.delete(add_pengaduan)
        db.session.commit()
...
def artikel():
    return render_template("artikel")
